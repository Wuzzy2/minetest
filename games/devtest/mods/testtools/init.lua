...
local function show_node_meta_formspec(user, key, value, keylist)
	local textlist
	if keylist then
...
minetest.register_tool("testtools:node_meta_editor", {
	...
	on_place = function(itemstack, user, pointed_thing)
		...
		show_node_meta_formspec(user, "", "", get_node_meta_keylist(meta, user:get_player_name()))
		return itemstack
	end,
})
	...
	elseif formname == "testtools:node_meta_editor" then
		...
		if fields.keylist then
			...
			if evnt.type == "DCL" or evnt.type == "CHG" then
				...
				show_node_meta_formspec(player, key, value, keylist)
				return
			end
		elseif fields.key and fields.key ~= "" and fields.value then
			...
			if fields.get then
				...
				show_node_meta_formspec(player, fields.key, value,
						get_node_meta_keylist(meta, name))
				return
			elseif fields.set then
				meta:set_string(fields.key, fields.value)
				show_node_meta_formspec(player, fields.key, fields.value,
						get_node_meta_keylist(meta, name))
				return
			end
		end
